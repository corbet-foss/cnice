...
The main branch ships 0.1.2 under LGPL-3.0-only WITH LGPL-3.0-linking-exception, published to registries. The existing releases documented below keep their original
...
[PyPI](https://pypi.org/project/cnice/0.1.2/). Install this release with pip
or uv in your Python environment:

```sh
python -m pip install cnice==0.1.2
```

```sh
uv pip install cnice==0.1.2
```

For a uv project, `uv add cnice==0.1.2` adds the package to your dependencies.
...
For an isolated CLI environment, use `pipx install cnice==0.1.2` or
`uv tool install cnice==0.1.2`.
...
[GitHub release](https://github.com/corbet-foss/cnice/releases/tag/v0.1.2).

## JSR

Version 0.1.2 is published as
[`@corbet-labs/cnice`](https://jsr.io/@corbet-labs/cnice@0.1.2):

```sh
deno add jsr:@corbet-labs/cnice@0.1.2
```

## Typst

Version 0.1.2 has not yet been submitted to
...
```typst
#import "@preview/cnice:0.1.2": *
```

For a local installation, download `cnice-0.1.2-typst.tar.gz` from the matching GitHub release and
extract its contents into `typst/packages/local/cnice/0.1.2` under your
