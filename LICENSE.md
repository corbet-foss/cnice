...
cnice 0.1.2 is licensed under **LGPL-3.0-only WITH LGPL-3.0-linking-exception**.
...
The `cgreet` and `cfarewell` repositories keep their original grants
untouched in their own repos (including their MIT/Apache-2.0 and earlier
release lines). This facade does not relicense them. Retained permissive
license texts in `LICENSES/` describe those sibling materials; they are not
alternative grants for new LGPL-covered work.
